...
~/.cache/project-setup/refresh. Apart from the pre-refresh backup described below,
no target files change in preparation/manual mode. The full prompt is printed for
copying into an agent, and its file path is shown.
...
Before any migration edit, refresh copies the whole existing project to
PROJECT/PROJECT.org inside the project itself (for example ABCD/ABCD.org). The
copy includes hidden files and .git but never the backup folder itself, and the
backup name is added to .git/info/exclude for repositories rooted at the project,
so it stays out of git status, diffs and commits. The migration prompt instructs
the agent to treat the backup as read-only and to keep it out of release-files.txt
and every migration mapping.

If PROJECT/PROJECT.org already exists it is kept unchanged and refresh continues
with it as the snapshot; delete it yourself if you want a fresh copy. A backup
interrupted mid-copy leaves PROJECT/PROJECT.org.partial, which is reported and
cleaned up automatically on the next run, so an existing .org directory is always
a complete backup.

The project size is measured before copying. Above 100 MB refresh prints a warning
with the measured size and asks for confirmation in a terminal: answer y/yes to
proceed with the copy, anything else stops the refresh before any project file
changes. Without a terminal, a project over 100 MB is refused rather than copied
unattended. The measured size includes .git history, so an old repository can
cross the limit even when its working files look small.
