...
body = body.replace('href="docs/refresh.md"', 'href="refresh.html"').replace('href="docs/LICENSING_OPTIONS.md"', 'href="LICENSING_OPTIONS.html"')
page = '''<!doctype html>
...
</style></head><body><header><p class="badge">macOS + Linux · Python 3.9+</p><h1>Projects, ready to build.</h1><p>project-setup 1.5.0 · Installation, Git setup, versioning and releases</p></header><main><nav><a href="usage.html">Step-by-step guide</a><a href="#install">Install</a><a href="#create-a-project">Create a project</a><a href="#data-workspace">Data workspace</a><a href="#git-setup">Git helpers</a><a href="#update-and-release">Releases</a><a href="#git-setup">Git reference</a></nav>'''+body+'''</main><footer>Offline user guide · project-setup · <a href="https://github.com/prasundutta151/project-setup">Source and releases</a></footer></body></html>'''
...
for name in ('refresh', 'LICENSING_OPTIONS'):
    content = (root/'docs'/f'{name}.md').read_text()
    (root/'docs'/f'{name}.txt').write_text(content)
    rendered = markdown.markdown(content, extensions=['fenced_code', 'tables', 'toc'])
    (root/'docs'/f'{name}.html').write_text('<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+name+'</title><style>body{max-width:1000px;margin:40px auto;padding:0 20px;font:16px/1.6 system-ui}pre{background:#edf2f4;padding:16px;overflow:auto}</style></head><body><nav><a href="index.html">Main documentation</a></nav>'+rendered+'</body></html>\n')

# Keep author attribution on every rebuilt HTML documentation page.
import sys
import json
sys.path.insert(0, str(root))
from project_setup.licensing import update_footers
metadata = root/'lisence/license-info.json'
if metadata.exists():
    update_footers(root, json.loads(metadata.read_text()))
